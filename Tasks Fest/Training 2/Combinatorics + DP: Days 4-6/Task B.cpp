...
void Solve() {
    ...
    Int good = Int(1) / Int(all);
    Int bad = Int(all - 1) / Int(all);
    ...
    vector<Int> pref_sf(k + 1);
    pref_sf[0] = Int(0);
    pref_sf[1] = Int(sf);
    for (int i = 2; i <= k; i++) {
        pref_sf[i] = pref_sf[i - 1] + Int(sf + (i - 1) * m);
    }

    for (int i = 1; i <= k; i++) {
        Int Cki(Find_C_n_k(k, i));
        Int gbf(good.pow(i) * bad.pow(k - i) * pref_sf[i]);
