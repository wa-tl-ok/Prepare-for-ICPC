...
void Solve() {
    ...
    vector<int> pref_sums = { 0 };
    int S = 0;
    for (int i = 1; i <= n; i++) {
        S += a[i];
        pref_sums.push_back(S);
    }
    set<int> dop;
    for (int i = 0; i <= n; i++) {
        dop.insert(pref_sums[i] - 1);
        dop.insert(pref_sums[i]);
        dop.insert(pref_sums[i] + 1);
    }
    map<int, int> ID;
    int cur = 0;
    ...
    vector<int> dp(n + 1);
    for (int i = 1; i <= n; i++) {
        dp[i] = max(dp[i - 1] + (a[i] < 0 ? -1 : a[i] > 0 ? 1 : 0), ST.query(0, ID[pref_sums[i] - 1]) + i);
        ST.change(ID[pref_sums[i]], max(dp[i] - i, ST.query(ID[pref_sums[i]], ID[pref_sums[i]])));
    }
