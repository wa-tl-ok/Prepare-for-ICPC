...
private:
    ...
    void BUILD_TRA() {
        ...
        while (Q.size() > 0) {
            //cout << "current node is " << Q.front()->num << endl;
            TrieNode* current = Q.front(); Q.pop();

            if (current == root) {
                suf_link[current] = root;
                //cout << "suf_link done for " << current->num << " ---> " << suf_link[current]->num << endl;
            }
            else {
            ...
            for (char c : alh) {
                if (sigma[{current, c}] == nullptr) {
                    ...
                    //cout << "sigma done for " << current->num << " and letter " << c << ' ' << " ---> " << sigma[{current, c}]->num << endl;
                }
                else {
                    //cout << "sigma WAS done for " << current->num << " and letter " << c << ' ' << " ---> " << sigma[{current, c}]->num << endl;
                }
            }
    ...
    */
    ...
    void solve() {
        ...
        for (auto u : suf_link) {
            ...
            //cout << u.second->num << " ---> " << u.first->num << endl;
        }
        //cout << endl;
        dfs(root);
    }
