...
private:
    ...
    void update(int x, int y, int val, int nodeX, int nodeY, int xLeft, int xRight, int yLeft, int yRight) {
        ...
        tree[nodeX][nodeY] = max(
            max(tree[nodeX * 2 + 1][nodeY * 2 + 1], tree[nodeX * 2 + 1][nodeY * 2 + 2]),
            max(tree[nodeX * 2 + 2][nodeY * 2 + 1], tree[nodeX * 2 + 2][nodeY * 2 + 2])
        );
    }

    int queryMax(int x1, int y1, int x2, int y2, int nodeX, int nodeY, int xLeft, int xRight, int yLeft, int yRight) {
        ...
        return max({
            queryMax(x1, y1, x2, y2, nodeX * 2 + 1, nodeY * 2 + 1, xLeft, midX, yLeft, midY),
            queryMax(x1, y1, x2, y2, nodeX * 2 + 1, nodeY * 2 + 2, xLeft, midX, midY + 1, yRight),
            queryMax(x1, y1, x2, y2, nodeX * 2 + 2, nodeY * 2 + 1, midX + 1, xRight, yLeft, midY),
            queryMax(x1, y1, x2, y2, nodeX * 2 + 2, nodeY * 2 + 2, midX + 1, xRight, midY + 1, yRight)
            });
    }
...
public:
    ...
    int queryMax(int x1, int y1, int x2, int y2) {
        return queryMax(x1, y1, x2, y2, 0, 0, 0, n - 1, 0, m - 1);
    }
};
